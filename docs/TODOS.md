...
## SYSTEM_PROMPT plus the tool catalog eat 74% of the context window

- **What:** Measured on the 9B at `CONTEXT_WINDOW = 8192`, on a job-search turn carrying no conversation history at all: `SYSTEM_PROMPT` is 3,641 tokens, the catalog of 15 tool schemas is 2,382, and the assembled hop-2 prompt is 6,228. That leaves 1,964 tokens for a reply. `MAX_NEW_TOKENS` was 2,048, so the app was asking for 84 more tokens than existed; it is now 1,536, which fits but leaves only about 400 tokens of slack.
- **Why:** Two consequences, one visible and one not. Every generation re-reads a 6,200-token prompt, which is why a job-search turn measured 301 seconds at 0.7 tok/s on a 4070 SUPER with full GPU offload: almost none of that is generation. And `Agent.max_history` keeps 8 messages, where a single job-search answer carrying a rendered posting list runs 500 to 800 tokens, so a few turns of conversation will push the prompt past 8,192 and answers will start being truncated for real.
- **Context:** Found while diagnosing a report of the assistant "cutting out" after three jobs. That turned out to be a different bug (a `<thinking>` block the prompt demanded, rendered as dimmed reasoning so the answer was invisible), but the measurement above came out of the same investigation and is the larger problem. Two cheap wins exist before anything structural: the tool catalog sends all 15 schemas on every turn including `cve_lookup` and `compare_two_documents` for a job query, worth roughly 1,500 tokens; and the rendered posting block is stored into history verbatim, so it is replayed on every subsequent turn even though `job_renderer` would rebuild it.
- **Blocked by:** nothing technical, but PR4 owns `SYSTEM_PROMPT` and should land first, since moving the prompt into `src/prompts/` is what makes its size measurable and reviewable in a diff.
