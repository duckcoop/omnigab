...
- **What:** `LINT_TARGETS = ["src", "scripts", "desktop_app.py"]` at `scripts/deploy.py:28`, which is not the `src tests` that `verify.bat`, `verify.sh`, and AGENTS.md section 7 all use. PR0a took that wider set from 111 findings to 51, because it cleaned `src` but never touched the other two targets. 48 residuals remain in `desktop_app.py` and `scripts/job_watcher.py`, and all of them are now cosmetic: 21 E127, 9 E231, 6 E306, 5 E702, 3 E303, 2 E301, 1 E401, 1 E128. Every pyflakes finding is gone, and `verify.bat` gates on `flake8 --select=F` across the whole repo to keep it that way.
...
- **Blocked by:** nothing, and no longer urgent: the remaining 48 findings cannot cause a failure at runtime.
